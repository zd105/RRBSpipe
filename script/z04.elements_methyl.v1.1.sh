...
## repeat
for j in DNA LINE LTR Retroposon RNA Satellite SINE Low_complexity Simple_repeat
do
	echo -e "##################################################\n|-----Start analysis $sample $j methylation level..."
	echo `date`
	perl $Bin/z04.elements_methyl.pl -c $cout -b $ele_dir/Repeats.$j.bed -t $type -o $outdir/$sample.Repeats.$j.methy.txt
	echo -e "[COMAND]: perl $Bin/z04.elements_methyl.pl -c $cout -b $ele_dir/Repeats.$j.bed -t $type -o $outdir/$sample.Repeats.$j.methy.txt"
	echo "-----|done..."
done
echo -e "|-----Violin plotting...\n"
echo -e "[COMAND]: $Rscript $Bin/z04.elements_methyl.repeats.R $outdir DNA-LINE-LTR-Retroposon-RNA-Satellite-SINE-Low_complexity-Simple_repeat $sample.genome.repeats $sample"
$Rscript $Bin/z04.elements_methyl.repeats.R $outdir DNA-LINE-LTR-Retroposon-RNA-Satellite-SINE-Low_complexity-Simple_repeat $outdir/$sample.genome.repeats $sample
