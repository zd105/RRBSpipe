...
my ($plot_script, $pval_out, $pval_bed, $pdf,$Rscript);
...
$USAGE = << "USE";
    ...
    usage:  perl $SCRIPTNAME  -q <query_file> [-o <path_prefix>] [-p  <number>] [-c <number>] [-d <number>] [-l <number>] [-a <string>] [-b <string>]
              ...
              This is a develope version of metilene_output.pl for Mann-Whitney-U test p-value filter instead of adj. p-value filter.
			  Author: zhangdu
    
    [INPUT]     -i          path/filename of metilene DMRs
                -o          path/prefix of output files (default: input_path/)
                -p          maximum (<) Mann-Whitney-U test p-value for output of significant DMRs (default: 0.05)
...
$pval_out = ($pval_out . "_pval." . $p_val . ".out");
$pval_bed = File::Spec->catpath($vol, $path, $file);
$pval_bed = ($pval_bed . "_pval." . $p_val . ".bedgraph");
...
open (IN, "$in") || die "cannot open $in\n";
    ...
    if (($MWU < $p_val) && (abs($diff_methyl) >= $min_diff) && ($nr_cpgs >= $min_cpgs) && (($end-$start) >= $length)) {
        print OUT "$chr\t$start\t$end\t$MWU\t$diff_methyl\t$nr_cpgs\t$mean_g1\t$mean_g2\n";
...
print STDERR ("[INFO]" . prettyTime() . "Wrote $count DMRs with Mann-Whitney-U test p-value<$p_val, a minimum absolute difference>=$min_diff, a minimum length [CpG]>=$min_cpgs and a minimum length [nt]>=$length\n");
