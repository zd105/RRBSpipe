...
#all sample plot
pdf(file=outpdf,height=5,width=0.7*n)
ggplot(tab, aes(x=Sample_ID, y=Methylation_level)) +
geom_violin(fill="deepskyblue",size=0.2) +
geom_boxplot(width=0.2/n,fill="black",outlier.size=1e-50,size=0.2) +
theme_minimal() +
stat_summary(fun.y=median, geom="point", size=12/n,shape=21,fill="white") +
