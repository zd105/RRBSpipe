...
#########################################
### This is a q-value fiter version of metilene_output.R
###
### Author: zhangdu
###
### Usage:  Rscript metilene_output.v_q-value_filter.R <dmr.txt> <outprefix> <group1 name> <group2 name>
#########################################
...
# 04 q_val vs difference
pdf(file = paste(args[2],"DMR_q_val-difference.pdf",sep="."),width =5,height=5)
ggplot(data, aes(x=diff, y=q_val)) + geom_point(alpha=.5) + scale_y_log10() + xlab("Mean methylation difference") + ylab("q-value") + labs(title=paste(args[3],"vs",args[4],"DMR q_val-difference",sep=" "))
#+ theme_cfg
dev.off()
