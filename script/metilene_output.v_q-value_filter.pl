...
my ($threads, $min_cpgs, $min_diff, $id1, $id2, $q_val, $length);
my ($plot_script, $qval_out, $qval_bed, $pdf,$Rscript);
...
$USAGE = << "USE";
    ...
    [INPUT]     -i           path/filename of metilene DMRs
                -o          path/prefix of output files (default: input_path/)
                -q          maximum (<) adj. p-value (q-value) for output of significant DMRs (default: 0.05)
...
## out file ##
$qval_out = File::Spec->catpath($vol, $path, $file);
$qval_out = ($qval_out . "_qval." . $q_val . ".out");
$qval_bed = File::Spec->catpath($vol, $path, $file);
$qval_bed = ($qval_bed . "_qval." . $q_val . ".bedgraph");
...
my $count = 0;
open(OUT, ">$qval_out") or die "##### AN ERROR has occurred: could not write to $qval_out\n";
open(OUT_B, ">$qval_bed") or die "##### AN ERROR has occurred: could not write to $qval_out\n";

open (IN, "$in") || die "cannot open $in\n";
    while (<IN>) {
    chomp;
    
    my ($chr, $start, $end, $qval, $diff_methyl, $nr_cpgs, $MWU, $KS2D, $mean_g1, $mean_g2) = split(/\t/,$_);
    if (($qval < $q_val) && (abs($diff_methyl) >= $min_diff) && ($nr_cpgs >= $min_cpgs) && (($end-$start) >= $length)) {
        print OUT "$chr\t$start\t$end\t$qval\t$diff_methyl\t$nr_cpgs\t$mean_g1\t$mean_g2\n";
...
print STDERR ("[INFO]" . prettyTime() . "Wrote $count DMRs with adj. p-value<$q_val, a minimum absolute difference>=$min_diff, a minimum length [CpG]>=$min_cpgs and a minimum length [nt]>=$length\n");
print STDERR ("[INFO]" . prettyTime() . "Bedgraph file containing DMR difference: $qval_bed\n");


#################
### plot stats ##
#################
#print "$plot_script $qval_out $pdf\n";
print STDERR ("[INFO]" . prettyTime() . "Plot DMR statistics.\n");
$call = ("$Rscript $plot_script $qval_out $pdf $id1 $id2 ");
call($call);
